...
class Synthesizer(object):
    ...
    def __enter__(self):
        ...
        try:
            ...
            assert (
                self.name in self.engine.scanner.available_instruments
            ...
            for effect_name in self.effects_name:
                assert (
                    effect_name in self.engine.scanner.available_effects
            ...
            self.synth = self.engine.pool.register_au_instrument(self.name)
            ...
            if self.use_mixer:
                self.output.connect(
                    self.engine.pool.main_mixer_node, from_bus=0, to_bus=0
                )
            else:
    ...
    def render(
        self,
        midi_setting_file=f"{get_config('default_midi_settings_args')['default_pitch']}.mid",
        no_cache=True,
    ):
        audio_output_file = RandString(16) + ".wav"
        temp = (True,)
        self.renderer.midi = mido.MidiFile(
            pjoin(self.midi_settings_dir, midi_setting_file)
        )
        default_midi_settings_args = get_config("default_midi_settings_args")
        self.renderer.render(
            pjoin(self.temp_output_dir, audio_output_file),
            start=0,
            end=default_midi_settings_args["recording_beats"]
            / default_midi_settings_args["bpm"]
            * 60,
        )
    ...
    def clear_cache(self, delete=False):
        Delete(self.temp_output_dir, rm=True) if delete else ClearFolder(
            self.temp_output_dir, rm=True
        )
...
class BaseInterface(object):
    ...
    def from_synth(cls, synth):
        normalize_function = {
            key: (lambda x: (x - param.min_value) / (param.max_value - param.min_value))
            for key, param in synth.parameters.items()
        }
        return cls(
            {
                key: normalize_function[key](param.current_value)
                for key, param in synth.parameters.items()
            }
        )
    ...
    def to_synth(self, synth):
        restore_function = {
            key: (lambda x: x * (param.max_value - param.min_value) + param.min_value)
            for key, param in synth.parameters.items()
        }
