...
class BaseInterface(object):
    ...
    def from_tensor(cls, data):
        ...
        for key, classes in cls.classification_parameters:
            result[key] = float(torch.argmax(data[index : index + classes])) / (
                classes - 1
            )
            index += classes
        ...
        return torch.cat(
            [
                F.one_hot(
                    torch.tensor(
                        [
                            Clipped(
                                int(round(self.data[key] * (classes - 1))),
                                0,
                                classes - 1,
                            )
                        ]
            ...
            + [
                F.one_hot(
                        ...
                        [
                            Clipped(
                                int(
                                    round(
                                        self.data[key]
                                        * (self.__class__.regression_nclasses - 1)
                                    )
                                ),
                                0,
                                self.__class__.regression_nclasses - 1,
                    ...
                    num_classes=self.__class__.regression_nclasses,
                )
                * 1.0
    ...
    def dim(cls):
        return sum(
            x[1] for x in cls.classification_parameters
        ) + cls.regression_nclasses * len(cls.regression_parameters)
    ...
    @classmethod
    def division(cls):
        return [x[1] for x in cls.classification_parameters] + [
            cls.regression_nclasses for x in cls.regression_parameters
        ]
    ...
    def named_division(cls):
        return {
            a[0]: a[1]
            for a in cls.classification_parameters
            + [(x, cls.regression_nclasses) for x in cls.regression_parameters]
        }
    ...
    def profile(cls):
        return {
            ...
            "# Regression Classes": cls.regression_nclasses
            * len(cls.regression_parameters),
            "# Classification Parameters": len(cls.classification_parameters),
            "# Classification Classes": sum(
                x[1] for x in cls.classification_parameters
            ),
...
class ParameterSpaceLoss(nn.Module):
    def __init__(
        self,
        default_loss_type="celoss",
        regression_parameters=list(),
        classification_parameters=list(),
        regression_nclasses=list(),
    ):
        super().__init__()
        self.default_loss_type = default_loss_type
        self.regp = regression_parameters
        self.clsp = classification_parameters
        self.regn = regression_nclasses

    def forward(self, pred, true, weights, loss_type=None):
        loss_type = self.default_loss_type if loss_type is None else loss_type
        clsl = LOSSES_MAPPING[loss_type]["clsl"]
        regl = LOSSES_MAPPING[loss_type]["regl"]
        index = 0
        result = {}
        keys = set()
        if clsl is not None:
            for key, classes in self.clsp:
                result[key] = clsl(
                    pred[:, index : index + classes], true[:, index : index + classes]
                )
                keys.add(key)
                index += classes
        ...
        if regl is not None:
            for key in self.regp:
                result[key] = regl(
                    pred[:, index : index + self.regn],
                    true[:, index : index + self.regn],
                )
                keys.add(key)
                index += self.regn
        losses = sum(result[key] * weights[key] for key in keys) / (
            sum(weights[key] for key in keys) + 1e-9
        )
        return losses.mean()
