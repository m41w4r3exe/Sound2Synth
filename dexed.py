...
def CreateDexedPresetAugmentDataset(
    presets,
    per_iterations,
    max_steps,
    dataset,
    ...
    from interface.dexed import (
        DexedInterface,
        regression_parameters,
        classification_parameters,
    )

    local_dst = p2s(pjoin(get_config("data_dir"), dataset), f=True)
    CreateFolder(local_dst)
    remote_dst = p2s(pjoin(get_config("server_data_dir"), dataset), f=True)
    h = RSYNC(local_dst, remote_dst, args="-q", wait=False)
    h.wait(timeout=5)
    rsync_failed = set()
            ...
            for i in range(per_iterations):
                ...
                for j in range(max_steps):
                    ...
                    if not flag:
                        ...
                        while (
                            key.startswith("OP")
                            and (not key.endswith("SWITCH"))
                            and params[key[:4] + "SWITCH"] < 0.5
                        ):
                            key = np.random.choice(keyset)
                    ...
                    if m > 0.01:
                        spbar.set_description(
                            "i[%02d]j[%02d]c[%06d][%.3f]" % (i, j, c, m)
                        )
                        ...
                        try:
                            h = RSYNC(
                                pjoin(local_dst, name + ".json"),
                                pjoin(remote_dst, name + ".json"),
                                args="-q",
                                wait=False,
                            )
                            h.wait(timeout=5)
                            h = RSYNC(
                                pjoin(local_dst, name + ".wav"),
                                pjoin(remote_dst, name + ".wav"),
                                args="-q",
                                wait=False,
                            )
                            h.wait(timeout=5)
                            Delete(pjoin(local_dst, name + ".wav"), rm=True)
                            Delete(pjoin(local_dst, name + ".json"), rm=True)
                        except:
...
def CreateDexedRandomDataset(per_iterations, max_steps, dataset, split, algorithm):
    CreateDexedPresetAugmentDataset(
        [None], per_iterations, max_steps, dataset, split, algorithm, False
    )
