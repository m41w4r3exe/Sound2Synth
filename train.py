...
if __name__ == "__main__":
    args = HeavenArguments.from_parser(
        [
            LiteralArgumentDescriptor(
                "synth", short="s", choices=INTERFACE_MAPPING.keys(), default="Dexed"
            ),
            LiteralArgumentDescriptor(
                "dataset_type",
                short="dt",
    ...
    # Prepare dataset
    dataset = DATASET_MAPPING[args.dataset_type](
        args.dataset, args.train_splits, with_params=True, augment=args.noise_augment
    )
    if args.split_train_to_val and args.split_train_to_test:
        args.datasets = SplitDatasets(
            dataset, train=0.7, val=0.2, test=0.1, cut=args.debug
        )
    elif args.split_train_to_val:
        args.datasets = SplitDatasets(
            dataset, train=0.8, val=0.2, test=0.0, cut=args.debug
        )
        args.datasets["test"] = DATASET_MAPPING[args.dataset_type](
            args.dataset, args.test_splits, with_params=True, augment=args.noise_augment
        )
    elif args.split_train_to_test:
        args.datasets = SplitDatasets(
            dataset, train=0.9, val=0.0, test=0.1, cut=args.debug
        )
        args.datasets["val"] = DATASET_MAPPING[args.dataset_type](
            args.dataset, args.val_splits, with_params=True, augment=args.noise_augment
        )
    else:
